...
export function computerAttack() {
  ...
  if (opponent.previousHits.length > 0) {
    ...
    //console.log(`Computer attacks cell (${smartCoord[0]}, ${smartCoord[1]}) on player's board.`);
    ...
    if (attackResult === "SUNK") {
      ...
      //console.log(`Computer is now switching to random attack. New random attack: ${randomCoord}`);
    }
  } else {
    // No hits to follow up on, perform random attack
    randomCoord = opponent.generateRandomAttack();
    //console.log(`Computer attacks cell (${randomCoord[0]}, ${randomCoord[1]}) on player's board.`);
